...
class HttpRequestDirector{
    public:
    static httpreq createGetRequest(const string& url){
    ...
    static httpreq createPostReq(const string& url, const string& jsonBody){
        return httpreqBuilder()
        .withurl(url)
        .withMethod("POST")
...
int main(){
    ...
    httpreq getreq = HttpRequestDirector ::createGetRequest("http://localhost:8080/api/login");
    getreq.execute();


    cout<<"--------------------------------------------------------------"<<endl;

    httpreq postreq = HttpRequestDirector ::createPostReq("http://localhost:8080/api/login", "this is create post req with request director");
    postreq.execute();
